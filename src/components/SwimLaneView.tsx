...
const SwimLaneView: React.FC<SwimLaneViewProps> = ({ workProcesses }) => {
  ...
  const ProcessCard = ({ process }: { process: WorkProcess }) => {
    const handleEdit = () => {
      // TODO: Open edit modal
      console.log('Edit process:', process.id);
    };

    // Handle status change via drag & drop (to be implemented)
    // const handleStatusChange = (newStatus: WorkProcess['status']) => {
    //   updateWorkProcess(process.id, { ...process, status: newStatus });
    // };
            ...
            return (
              <div key={category.id} className="flex border-b border-gray-100 hover:bg-gray-50">
                ...
                {columns.map(column => {
                  ...
                  return (
                    <div key={column.id} className={cn('flex-1 p-3 border-r border-gray-100', column.color)}>
